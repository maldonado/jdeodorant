...
public class ASTReader {
	...
	private ClassObject processTypeDeclaration(IFile iFile, IDocument document, TypeDeclaration typeDeclaration, List<Comment> comments) {
		...
		try {
			...
			int startLine = document.getLineOfOffset(startPosition);
			int endLine = document.getLineOfOffset(startPosition + typeDeclaration.getLength());
			classObject.setStartLine(startLine);
			classObject.setEndLine(endLine);
		} catch (BadLocationException e) {
			e.printStackTrace();
		}
	...
	private ClassObject processEnumDeclaration(IFile iFile, IDocument document, EnumDeclaration enumDeclaration, List<Comment> comments) {
		...
		try {
			int startPosition = enumDeclaration.getStartPosition();
			int startLine = document.getLineOfOffset(startPosition);
			int endLine = document.getLineOfOffset(startPosition + enumDeclaration.getLength());
			classObject.setStartLine(startLine);
			classObject.setEndLine(endLine);
		} catch (BadLocationException e) {
			e.printStackTrace();
